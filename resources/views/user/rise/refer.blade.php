...
<div class="am-body">
    ...
    {{-- Welcome Message --}}
    <div style="font-size:18px;font-weight:700;color:#fff;padding:0 2px;">{{ __('Welcome back') }}, {{ $user->fullname ?? $user->username }}!</div>

    {{-- Stats Row --}}
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
        <div style="background:#111827;border:1px solid #1E293B;border-radius:14px;padding:16px 12px;text-align:center;">
            <div style="font-size:24px;font-weight:800;color:#fff;">{{ $referral_count ?? 0 }}</div>
    ...
    <div class="ps-card">
        ...
        <p style="font-size:13px;color:#94A3B8;margin:-8px 0 12px;">{{ __('Share this link with friends to earn rewards.') }}</p>
        <div style="display:flex;gap:8px;">
            <input type="text" id="referLinkInput" value="{{ $referralLink }}" readonly
                   style="flex:1;padding:12px 14px;border:1px solid #334155;border-radius:10px;font-size:13px;background:#1E293B;color:#fff;outline:none;">
            <button onclick="copyRefLink()" id="copyRefBtn"
                    style="padding:12px 18px;background:#3B82F6;color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;">{{ __('Copy') }}</button>
        </div>
        <div style="display:flex;gap:8px;margin-top:10px;">
            <button onclick="shareRefLink()" style="flex:1;padding:10px;border-radius:10px;background:#1E293B;color:#3B82F6;font-size:13px;font-weight:600;border:1px solid #334155;cursor:pointer;">{{ __('Share via') }} WhatsApp</button>
            <button onclick="shareRefSMS()" style="flex:1;padding:10px;border-radius:10px;background:#1E293B;color:#3B82F6;font-size:13px;font-weight:600;border:1px solid #334155;cursor:pointer;">{{ __('Share via') }} SMS</button>
        </div>
    </div>
    ...
    <div class="ps-card">
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div style="display:flex;gap:14px;align-items:flex-start;">
                ...
                <div>
                    <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:2px;">{{ __('Share Your Link') }}</div>
                    <div style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Send your unique referral link to friends via WhatsApp, SMS, email, or social media.') }}</div>
                </div>
            </div>
            {{-- Step 2 --}}
            <div style="display:flex;gap:14px;align-items:flex-start;">
                ...
                <div>
                    <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:2px;">{{ __('Friend Signs Up') }}</div>
                    <div style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('They register using your link and create an account. You\'re credited as their referrer.') }}</div>
                </div>
            </div>
            {{-- Step 3 --}}
            <div style="display:flex;gap:14px;align-items:flex-start;">
                ...
                <div>
                    <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:2px;">{{ __('They Deposit $600+') }}</div>
                    <div style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Your friend adds $600 or more to their account. Both of you unlock withdrawal access.') }}</div>
                </div>
            </div>
            {{-- Step 4 --}}
            <div style="display:flex;gap:14px;align-items:flex-start;">
                ...
                <div>
                    <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:2px;">{{ __('You Earn $50') }}</div>
                    <div style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Once their deposit is confirmed, $50 is credited to your wallet. Refer more, earn more!') }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="ps-card">
        ...
        <p style="font-size:13px;color:#94A3B8;margin:-8px 0 12px;">{{ __('Referral earnings can be withdrawn once your referred friends have met the deposit requirement.') }}</p>
        <div style="display:flex;flex-direction:column;gap:14px;">
            <div style="display:flex;gap:12px;align-items:flex-start;">
                ...
                <span style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Ensure at least one referred friend has deposited $600 or more (status: completed).') }}</span>
            </div>
            <div style="display:flex;gap:12px;align-items:flex-start;">
                ...
                <span style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Go to the Withdraw page and select your preferred withdrawal method.') }}</span>
            </div>
            <div style="display:flex;gap:12px;align-items:flex-start;">
                ...
                <span style="font-size:13px;color:#94A3B8;line-height:1.4;">{{ __('Enter the amount you wish to withdraw (up to your available referral earnings).') }}</span>
            </div>
            <div style="display:flex;gap:12px;align-items:flex-start;">
    ...
    <div class="ps-card">
        <div class="ps-card-title">{{ __('Your Referrals') }} <span style="font-size:12px;font-weight:500;color:#94A3B8;">({{ $referredUsers->count() }})</span></div>
        <div style="display:flex;flex-direction:column;">
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #1E293B;">
                ...
                <div style="flex:1;min-width:0;">
                    <div style="font-size:14px;font-weight:600;color:#fff;">{{ $ref->fullname ?? $ref->username }}</div>
                    <div style="font-size:12px;color:#4B5563;">{{ $ref->email }}</div>
                </div>
                <div style="font-size:12px;color:#4B5563;white-space:nowrap;">{{ $ref->created_at->format('M d, Y') }}</div>
            </div>
            @endforeach
        </div>
    </div>
    @else
    <div style="display:flex;flex-direction:column;align-items:center;padding:40px 20px;text-align:center;gap:8px;background:#111827;border:1px solid #1E293B;border-radius:16px;">
        <div style="font-size:40px;line-height:1;">👥</div>
        <div style="font-size:16px;font-weight:700;color:#fff;">{{ __('No referrals yet') }}</div>
        <div style="font-size:13px;color:#94A3B8;max-width:280px;">{{ __('Share your referral link with friends and family to start earning $50 per referral.') }}</div>
    </div>
    @endif
