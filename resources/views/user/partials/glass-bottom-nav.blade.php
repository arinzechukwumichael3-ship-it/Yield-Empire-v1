...
<nav class="glass-nav" role="navigation" aria-label="Main navigation">
    <div class="glass-nav-inner">
        ...
        <div class="glass-nav-group">
            <a href="{{ route("user.rise.feed") }}" class="glass-nav-item {{ request()->routeIs(["user.rise.feed", "user.transactions.*", "user.rise.send", "user.rise.withdraw.crypto"]) ? "active" : "" }}" aria-label="Feed">
            ...
            <a href="{{ route("user.loans.index") }}" class="glass-nav-item {{ request()->routeIs(["user.loans.*"]) ? "active" : "" }}" aria-label="Loans">
                <svg class="glass-nav-icon" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="6" width="18" height="13" rx="2"/><path d="M3 10h18"/><path d="M7 15h4"/></svg>
            </a>
